...
// import org.sistemadegestiondelesionescutaneas.repository.ImagenInfo // Si se usa la proyección ImagenInfo
...
class ImagenController {
    ...
    fun handleFileUpload(
        ...
        // Usuariorepositorio.findByUsuario ahora usa @EntityGraph, por lo que perfilPaciente se carga eficientemente.
        ...
        val paciente: Paciente = usuario.perfilPaciente // Esto ya no causa una consulta N+1 gracias a @EntityGraph
            ?: run { //
    ...
    fun mostrarHistorial(
        ...
        val paciente: Paciente? = usuario.perfilPaciente // Esto ya no causa una consulta N+1
        if (paciente == null) { //
        ...
        try {
            // Para usar la proyección, cambiarías:
            // val imagenes: List<ImagenInfo> = imagenLesionRepositorio.findProjectedByPacienteOrderByFechaSubidaDesc(paciente)
            // y ajustarías el modelo y la plantilla Thymeleaf.
            // Por ahora, mantenemos ImagenLesion completa ya que la plantilla lo espera.
    ...
    // El método serveFile y deleteImage ya usan optimizaciones (delete usa el método optimizado del servicio)
    // o son estándar y generalmente eficientes.
    // ... (resto de los métodos: serveFile, deleteImage, companion object)
    ...
    fun deleteImage(
        ...
        val username = authentication.name //
        // Se asume que el servicio de autenticación o UserDetailsService carga el Usuario completo.
        // El método delete del servicio ya usa la carga optimizada.
